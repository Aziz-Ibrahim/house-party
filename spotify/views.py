...
class CurrentSong(APIView):
    """
    View to get the currently playing song.
    """
    def get(self, request, format=None):
        """
...
class PauseSong(APIView):
    """
    View to pause the currently playing song.
    """
    def put(self, response, format=None):
        """
        Handle PUT requests to pause the current song.
        """
        room_code = self.request.session.get('room_code')
        ...
        return Response(
            {'message': 'You are not allowed to pause the song.'},
...
class PlaySong(APIView):
    ...
    def put(self, response, format=None):
        """
        Handle PUT requests to play a song.
        """
        room_code = self.request.session.get('room_code')
        room = Room.objects.filter(code=room_code)[0]
        if self.request.session.session_key == room.host or room.guest_can_pause:
